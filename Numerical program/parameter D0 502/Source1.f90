...
! scan the parameter space
	...
	INTEGER IPOINT, IPAR1, IPAR2, CCi,CTi,TCi,Ti,CSi,TSi,times
	REAL(8) CHISQ,testTT,testTS
...
! We only need to change the parameters here
	...
	Isu = 10! 1 for pion, 2 for Kaon, 3 for proton, 4 for lambda^0, 5 for Xi, 6 for Phi, 7 for Omega, 
			! 8 for Jpsi, 9 for D^0, 10 for D_s
	IDparticle = Isu

    IYangZhu = 2 ! =1 use Yang's equations,  =2 use Zhu's equations
    ISys = 2 ! =1 for AuAu, =2 for PbPb at 2.76, =3 for PbPb at 5.02, =4 for XeXe at 5.44, =5 for dAu, =6 for pPb
...
! D_s
	...
	print*, 'The is for Ds case'
         ...
         if(c==0.05D0)then
			open(unit = 11, file = "err of dNptdpt_"//CollSys//"_Ds_c025 final.txt", status = "unknown")
			if(ISys.eq.3)then
				open(unit=101, file='./data_Ds_502.dat', status='old')
			elseif(ISys.eq.2)then
				open(unit=101, file='./data_Ds_276.dat', status='old')
            endif
		elseif(c==0.35D0.or.c==0.4D0)then
				open(unit = 11, file = "err of dNptdpt_"//CollSys//"_Ds_c35 final.txt", status = "unknown")
                
                if(ISys.eq.3)then
				open(unit=101, file='./data_Ds_502_40.dat', status='old')
				elseif(ISys.eq.2)then
...
! we need to modify here to choose the pt range
	...
	CHISQ = CHISQ + (Dlog10(dNptdpt(5)/DATAY(I)))**2

!	print *, pt, (dNptdpt(2:3))
	
	!write(*, 110) pt, dNptdpt(1:5)
	!write(114514, 110) pt, dNptdpt(1:5)
...
!----------------------------------------------------------		
    ...
    !保证TT与TS组份的焦点在pt = 10以前
	!pt = 10D0	
	!testTT  = meson_TT(pt, IDP1, IDP2, Isu) 
	!testTS  = meson_TS(pt, IDP1, IDP2, Isu) 
    !if(testTT>=testTS)then
    !    CHISQ = CHISQ+100D0
    !endif
...
! the minijet distribution 1/(2piptdptdy)
    ...
    real*8 A, bT, b, c, d, sigma ,Ncoll!the parameter of fik for charm 
...
!=============================================
    ! 4.058D0/26.2D0 *  for 30% -- 50%
    ! 7.109D0/26.2D0  *  for 20% -- 40%
    ! 5.259D0/26.2D0  *  for 35% Ds
        
    ! 23.37D0/26.2D0  *	for 0--10%
...
! c, cbar quark
	...
	fik =Ncoll* A/sigma * dexp(-bT* DATAN(k/b))/(1+(k/b)**d)**c
	
    end select
!=============================================
		! change the fi(k , c) to TAA(c)/TAA(c = 0.025) * fi(k , c = 0.025)
!==============================================
		if (  centr== 0.05D0 )then
...
!========================
    ...
    if(ISys.eq.3)then
!========================================================
	!  3.917D0/26.08D0 * for 30% -- 50% 
    !  6.927D0/26.08D0 * for 20%--40%
        
    !	23.26D0/26.08D0 * for 0--10%
!===========================================================
