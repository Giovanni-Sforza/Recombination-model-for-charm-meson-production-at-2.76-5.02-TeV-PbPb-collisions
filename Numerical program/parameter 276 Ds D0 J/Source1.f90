...
! We only need to change the parameters here
	...
	Isu = 9! 1 for pion, 2 for Kaon, 3 for proton, 4 for lambda^0, 5 for Xi, 6 for Phi, 7 for Omega, 
...
! D^0
	...
	print*, 'The is for D0 case'
         ...
         if(c==0.05D0)then
	 open(unit = 11, file = "err of dNptdpt_"//CollSys//"_D0_c025 final.txt", status = "unknown")
   
    elseif(c==0.4D0)then
    open(unit = 11, file = "err of dNptdpt_"//CollSys//"_D0_c40 final.txt", status = "unknown")
...
!276
     ...
     if(c==0.05D0)then
	 open(unit=101, file='./data_D0_276.dat', status='old')
   
    elseif(c==0.4D0)then
    open(unit=101, file='./data_D0_276_40.dat', status='old')
