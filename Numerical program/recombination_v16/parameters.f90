...
! scan the parameter space
	...
	INTEGER IPOINT, IPAR1, IPAR2, CCi,CTi,TCi,Ti
	REAL(8) CHISQ
...
! Jpsi
	...
	open(unit = 11, file = "dNptdpt_"//CollSys//"_Jpsi_c025.txt", status = "unknown")
...
! D^0
	...
	open(unit = 11, file = "dNptdpt_"//CollSys//"_D0_c40.txt", status = "unknown")
...
! D_s
	...
	open(unit = 11, file = "dNptdpt_"//CollSys//"_Ds_c025.txt", status = "unknown")
...
!276
        if(Isu.eq.8)then
	open(unit=101, file='./data_Jpsi_276.dat', status='old')
		elseif(Isu.eq.9)then
...
!502
		if(Isu.eq.8)then
	open(unit=101, file='./data_Jpsi_502.dat', status='old')
		elseif(Isu.eq.9)then
    open(unit=101, file='./data_D0_502.dat', status='old')
		elseif(Isu.eq.10)then
...
! T and T_s for 2.76 and 5.02 TeV are from Eqs. (4)-(7) of HZ2018, PRC97, 054902(2018)
	T = 0.2D0 + Ti * 0.02D0                           !0.39D0
	cT= 20D0 + CTi * 0.5D0          !23.2D0    ! c=0.025

	Ts = 0.51D0
	cS = 11.0D0    ! c=0.025

    !Tc & Cc
    Tc = 0.4D0 + TCi * 0.05D0                         !0.68d0          !0.73d0!0.7
    cc = 0.6d0 + CCi * 0.02D0 !0.8D0   !2.5d0!2.7
    !Tc = 0.5D0+IPAR1*0.1d0
	!cc = 0.1D0+IPAR2*0.1d0    ! c=0.025
    
elseif(ISys.eq.3)then
 ! parameters for Pb-Pb collisions at 5.02 TeV ! lilin May-16-2020
         T = 0.415D0
         cT=22.0D0    ! c=0.025   ! We search the parameters cT and cS in v15
		 ...
		 !Tc & Cc
		 Tc = 0.83d0!0.83d0
		 cc = 0.5d0! 2.2d0   ! c=0.025
         !Tc = 0.5D0+IPAR1*0.1d0
         !cc = 0.5D0+IPAR2*0.2d0    ! c=0.025
endif
...
!   For Pb+Pb, 2.76 TeV JPG47, 055102(2020)
    gamai = 2.8d0/(1.0d0+q**2/49.0d0) ! consider the gammai depends on q
        elseif(Isys.eq.3)then
	gamai = 4.5d0/(1.0d0+q**2/49.0d0) ! consider the gammai depends on q
...
!	gamai = 0.07d0
        if(Isys.eq.2)then
!   For Pb+Pb, 2.76 TeV JPG47, 055102(2020)
    gamai = 2.8d0/2.0d0/(1.0d0+q**2/49.0d0) ! consider the gammai depends on q
		elseif(Isys.eq.3)then
!   For Pb+Pb, 5.02 TeV
	gamai = 4.5d0/2.0d0/(1.0d0+q**2/49.0d0)
        else
