...
!-------------------------------------------
    ...
    character(len=4) CollCentrality
...
! scan the parameter space
    REAL*8 DATAX(500), DATAY(500), xpt(9),DATAterr(500)
...
! We only need to change the parameters here
	...
	c = 0.35D0        ! centrality
	Npart = 382.7D0   ! N participant from experimental data, only ResPi use

	Isu = 3! 1 for pion, 2 for Kaon, 3 for proton, 4 for lambda^0, 5 for Xi, 6 for Phi, 7 for Omega, 
			! 8 for Jpsi, 9 for D^0, 10 for D_s
	IDparticle = Isu
...
!276
        if(Isu.eq.8)then
            ...
            open(unit=101, file='./data_Jpsi_276_30.dat', status='old')
        elseif(Isu.eq.9)then
            if(c==0.05D0)then
				open(unit=101, file='./v2 D0 276 010.dat', status='old')
            elseif(c==0.40D0)then
                open(unit=101, file='./v2 D0 276 3050.dat', status='old')
            endif
		elseif(Isu.eq.10)then
			open(unit=101, file='./data_Ds_276_35.dat', status='old')
		
        
		elseif(Isu.eq.1)then
			if(c == 0.025D0)then 
				open(unit=101, file='./v2 pion 276 05.dat', status='old')    
			elseif(c == 0.075D0)then 
				open(unit=101, file='./v2 pion 276 510.dat', status='old')   
			elseif(c == 0.15D0)then 
				open(unit=101, file='./v2 pion 276 1020.dat', status='old')  
			elseif(c == 0.25D0)then 
				open(unit=101, file='./v2 pion 276 2030.dat', status='old')   
			elseif(c == 0.35D0)then 
				open(unit=101, file='./v2 pion 276 3040.dat', status='old')
			elseif(c == 0.45D0)then 
				open(unit=101, file='./v2 pion 276 4050.dat', status='old')  
			elseif(c == 0.55D0)then 
				open(unit=101, file='./v2 pion 276 5060.dat', status='old') 
            endif
            
        elseif(Isu.eq.3)then
			if(c == 0.025D0)then 
				open(unit=101, file='./v2 proton 276 05.dat', status='old')    
			elseif(c == 0.15D0)then 
				open(unit=101, file='./v2 proton 276 1020.dat', status='old')  
			elseif(c == 0.25D0)then 
				open(unit=101, file='./v2 proton 276 2030.dat', status='old')   
			elseif(c == 0.35D0)then 
				open(unit=101, file='./v2 proton 276 3040.dat', status='old')
            endif
...
!502
			if(Isu.eq.8)then
				open(unit=101, file='./v2 Jpsi 502 30.dat', status='old')
			elseif(Isu.eq.9)then
				open(unit=101, file='./v2 D0 502 3050.dat', status='old')
			elseif(Isu.eq.10)then
				open(unit=101, file='./v2 Ds 502 40.dat', status='old')
		
			elseif(Isu.eq.1)then			
					if(c == 0.025D0)then 
					open(unit=101, file='./v2 pion 502 05.dat', status='old') 
					elseif(c == 0.15D0)then 
						open(unit=101, file='./v2 pion 502 1020.dat', status='old')    
					elseif(c == 0.25D0)then 
						open(unit=101, file='./v2 pion 502 2030.dat', status='old')   
					elseif(c == 0.35D0)then 
						open(unit=101, file='./v2 pion 502 3040.dat', status='old') 
					elseif(c == 0.45D0)then 
						open(unit=101, file='./v2 pion 502 4050.dat', status='old')   
					endif
            
            elseif(Isu.eq.3)then
				if(c == 0.025D0)then 
					open(unit=101, file='./v2 proton 502 05.dat', status='old')    
				elseif(c == 0.15D0)then 
					open(unit=101, file='./v2 proton 502 1020.dat', status='old')  
				elseif(c == 0.25D0)then 
					open(unit=101, file='./v2 proton 502 2030.dat', status='old')   
				elseif(c == 0.35D0)then 
					open(unit=101, file='./v2 proton 502 3040.dat', status='old')
                endif
            endif
    endif
    
	open(unit=102, file='chisq.dat',status='unknown')

	IPOINT = 1
	
200	READ(101, *, END=210)DATAX(IPOINT), DATAy(IPOINT), DATAterr(IPOINT)
...
!CT , CS are interpolated in parameter.py, while CC TC are selected by scaning_parameter.f90 
    ...
    if(c==0.025D0)then
        !parameter Gonghuanjing
        CT =23.2D0
        CS =11D0
        
        Cc = 0.8d0
		Tc = 0.68D0
    elseif(c==0.05D0.or.c==0.1D0)then
        CT =22.434962D0
        CS =10.616995D0
    ...
    elseif(c==0.075D0)then
        ! for pion
        CT =21.6821992481203D0
        CS =10.246308099794941D0
    ...
    !for pion
        ...
        write(*,*) "this centrality have not been interpolated"
        stop
    endif
...
!  The statistical factors for the three particles are from HZ2020, JPG47, 055102(2020)
    ...
    elseif(IDParticle.eq.8) then
        
    gbst = 1.0D0
    
    elseif(IDParticle.eq.9) then
        
    gbst = 2.3D0
    
    elseif(IDParticle.eq.10) then
...
!    do IPAR2 = 0, 10
!
!	PAR(1) = 1.0D0+IPAR1*0.5d0
!	PAR(2) = 5.0D0+IPAR2*1.0d0
!-------------------------------------------
...
! pion
	...
	open(unit = 11, file = "dNptdpt_"//CollSys//"_pion_c"//CollCentrality//"c.txt", status = "unknown")
...
! proton
	...
	open(unit = 11, file = "dNptdpt_"//CollSys//"_proton_c"//CollCentrality//".txt", status = "unknown")
...
! Jpsi
	...
	print*, 'The is for Jpsi case'
	open(unit = 11, file = "dNptdpt_"//CollSys//"_Jpsi_c"//CollCentrality//".txt", status = "unknown")
...
! D^0
	...
	print*, 'The is for D0 case'
	open(unit = 11, file = "dNptdpt_"//CollSys//"_D0_c"//CollCentrality//".txt", status = "unknown")
...
! D_s
	...
	print*, 'The is for Ds case'
	open(unit = 11, file = "dNptdpt_"//CollSys//"_Ds_c"//CollCentrality//".txt", status = "unknown")

	endif

!---------------------------------------------	
	if(Isu.eq.1.or.Isu.eq.2.or.Isu.eq.6.or.Isu.eq.8.or.Isu.eq.9.or.Isu.eq.10)then
	write(11, 108)
...
! the xpt() is for the calculation of v2
	CHISQ = 0.0
	xpt(1)=1.03D0		!Jpsi 502 c40	!3.186D0	!Ds 502 c40		!1.778D0	!D0 502 c40		!1.14   !Jpsi 276 c30	!2.499	!D0 276 40	!2.494D0	! D0 276 c025	!
	xpt(2)=3.17D0		!Jpsi 502 c40	!4.846D0	!Ds 502 c40		!2.501D0	!D0 502 c40		!2.833  !Jpsi 276 c30	!3.458	!D0 276 40	!3.506D0	! D0 276 c025	!
	xpt(3)=5.2D0		!Jpsi 502 c40	!6.819D0	!Ds 502 c40		!3.47D0     !D0 502 c40		!4.821  !Jpsi 276 c30	!4.859	!D0 276 40	!4.848D0	! D0 276 c025	!
    xpt(4)=0			!Jpsi 502 c40   !9.599D0	!Ds 502 c40		!4.433D0	!D0 502 c40		!7.216  !Jpsi 276 c30	! 0		!D0 276 40	!6.861D0	! D0 276 c025	!
	xpt(5)=0	 !5.463D0	!D0 502 c40	
    xpt(6)=0	 !6.453D0	!D0 502 c40	
    xpt(7)=0	 !7.473D0	!D0 502 c40	
    xpt(8)=0		!8.898D0	!D0 502 c40	
    xpt(9)=0		!10.9D0	!D0 502 c40	
    ...
    do i = 1, IPOINT
    	pt = DATAX(i)
	!do i = 1, 60
    !  pt =0.0001D0 +  (i-1D0)*0.25d0
    !do i = 1,40
	!	pt = 0.0001D0  + (i-1D0)*0.06d0
    !do i = 1,9
    !   pt = xpt(i)
...
!	print *, pt, dNptdpt(2),  dNptdpt(3)
	...
	PRINT *, "cc,Tc: ", cc, Tc, PAR(1), PAR(2), CHISQ
	WRITE(102, *)cc, Tc, PAR(1), PAR(2), CHISQ
...
!------------------------------------------------------	
    ...
    if(IDP1.eq.8.or.IDP1.eq.9)then
...
!------------------------------------
    ...
    if(IDP1.eq.8.or.IDP2.eq.8)then
    IDH = 5
...
! meson spectrum from TT
	...
	external Intf_kaonTT, Intf_D0TT, Intf_DsTT
	real*8 Intf_kaonTT, Intf_D0TT, Intf_DsTT
...
!-------------------------------------------	
    ...
    meson_TT = 12.0*cT*cS/p0/pt**5*fsumf
	
	elseif(Isu.eq.6)then ! for phi
	
	meson_TT = gbst*cS**2*pt/4.0D0/p0*dexp(-pt/Ts)
    
    elseif(Isu.eq.8)then ! for Jpsi
	
	meson_TT = gbst*cc**2*pt/4.0D0/p0*dexp(-pt/Tc)
    !print *, pt, meson_TT
    
    elseif(Isu.eq.9)then ! for D^0
	
	call  Integral_1d(Intf_D0TT, fa, fb, fdel, fp1, fp2, fp3, fpt, fIDP1, fIDP2, fIDP3, fcentr, fsumf)
		
    !meson_TT = 5.0d0*gbst*cT*cc/p0/pt**6*fsumf
    meson_TT = 12.0d0*gbst*cT*cc/p0/pt**5*fsumf
    
    elseif(Isu.eq.10)then ! for D_s
	
	call  Integral_1d(Intf_DsTT, fa, fb, fdel, fp1, fp2, fp3, fpt, fIDP1, fIDP2, fIDP3, fcentr, fsumf)
		
    !meson_TT = 660.0d0*gbst*cc*cS/p0/pt**13*fsumf
    meson_TT = 60.0d0*gbst*cc*cS/p0/pt**7*fsumf
...
!---------------------------------------------------
! integral function for D0 from TT
! inputs, x, pt

	function Intf_D0TT(x, p1, p2, p3, pt, IDP1, IDP2, IDP3, centr)
	implicit none
	include 'Recom.h'
	real*8 Intf_D0TT
	real*8 x, p1, p2, p3, pt
...
!--------------------------------------------
	...
	!Intf_D0TT = x*dexp(-x/T)*(pt-x)*dexp(-(pt-x)/Tc)	*(pt-x)**4
	Intf_D0TT = x*(pt-x)**2*x*dexp(-x/T)*(pt-x)*dexp(-(pt-x)/Tc)
	return
...
! inputs, x, pt
	...
	function Intf_DsTT(x, p1, p2, p3, pt, IDP1, IDP2, IDP3, centr)
	implicit none
	include 'Recom.h'
	real*8 Intf_DsTT
	real*8 x, p1, p2, p3, pt
...
!--------------------------------------------
	...
	!Intf_DsTT = x*dexp(-x/Ts)*(pt-x)*dexp(-(pt-x)/Tc) *x**2 *(pt-x)**9
    Intf_DsTT = x**2*(pt-x)**3*x*dexp(-x/Ts)*(pt-x)*dexp(-(pt-x)/Tc)
...
! meson spectrum from TS
	...
	external Intf_pionTS, Intf_kaonTS, Intf_D0TS, Intf_DsTS
	real*8 Intf_pionTS, Intf_kaonTS, Intf_D0TS, Intf_DsTS
	
	external f_PhiTS, f_JpsiTS
	real*8 f_PhiTS, f_JpsiTS
...
!------------------------------------------------
	...
	meson_TS = gbst*cc/2.0D0/p0*f_JpsiTS(fpt, fIDP1, fIDP2, fIDP3, fcentr)
    
    elseif(Isu.eq.9)then ! for D^0
	
	call Integral_1d(Intf_D0TS, fa, fb, fdel, fp1, fp2, fp3, fpt, fIDP1, fIDP2, fIDP3, fcentr, fsumf)
	
	!meson_TS = 5.0D0*gbst/p0/pt**6*fsumf
    meson_TS = 12.0D0*gbst/p0/pt**5*fsumf
    
    elseif(Isu.eq.10)then ! for D_s
	
	call Integral_1d(Intf_DsTS, fa, fb, fdel, fp1, fp2, fp3, fpt, fIDP1, fIDP2, fIDP3, fcentr, fsumf)
	
	meson_TS = 60.0D0*gbst/p0/pt**7*fsumf
...
!---------------------------------------------------
! function for Jpsi from TS
! inputs, pt, IDP1, IDP2, centr
	
	function f_JpsiTS(pt, IDP1, IDP2, IDP3, centr)
	implicit none
	include 'Recom.h'
	real*8 f_JpsiTS
	real*8 pt
...
!--------------------------------------------
	...
	f_JpsiTS = dexp(-p2/Tc)*HardInt(IDP1, p1, centr)
...
!---------------------------------------------------
! function for D0 from TS
! inputs, pt, IDP1, IDP2, centr
	
	function Intf_D0TS(x, p1, p2, p3, pt, IDP1, IDP2, IDP3, centr)
	implicit none
	include 'Recom.h'
	real*8 Intf_D0TS
	real*8 x, p1, p2, p3, pt
...
!--------------------------------------------
	...
	!Intf_D0TS = x*(pt-x)**4*cT*dexp(-x/T)*HardInt(IDP2, pt-x, centr)
	!Intf_D0TS = Intf_D0TS + x*(pt-x)**4*HardInt(IDP1, x, centr)*cc*(pt-x)/x*dexp(-(pt-x)/Tc)	 
	Intf_D0TS = x**2*(pt-x)**2*cT*dexp(-x/T)*HardInt(IDP2, pt-x, centr)
	Intf_D0TS = Intf_D0TS + x**2*(pt-x)**2*HardInt(IDP1, x, centr)*cc*(pt-x)/x*dexp(-(pt-x)/Tc)	
...
!---------------------------------------------------
! function for Ds from TS
! inputs, pt, IDP1, IDP2, centr
	
	function Intf_DsTS(x, p1, p2, p3, pt, IDP1, IDP2, IDP3, centr)
	implicit none
	include 'Recom.h'
	real*8 Intf_DsTS
	real*8 x, p1, p2, p3, pt
...
!--------------------------------------------
	...
	!Intf_DsTS = x**3*(pt-x)**9*cS*dexp(-x/Ts)*HardInt(IDP2, pt-x, centr)
	!Intf_DsTS = Intf_DsTS + x**3*(pt-x)**9*HardInt(IDP1, x, centr)*cc*(pt-x)/x*dexp(-(pt-x)/Tc)
    Intf_DsTS = x**3*(pt-x)**3*cS*dexp(-x/Ts)*HardInt(IDP2, pt-x, centr)
	Intf_DsTS = Intf_DsTS + x**3*(pt-x)**3*HardInt(IDP1, x, centr)*cc*(pt-x)/x*dexp(-(pt-x)/Tc)
...
!--------------------------------------------
    ...
    if(IDparticle.eq. 8 .or. IDparticle.eq.9 .or. IDparticle .eq.10)then
    IDH = 5
	Intf_mesonSS = Intf_mesonSS + Fiqc(IDH, x, centr)*QuenchFiqc(x)*FF(IDH, IDparticle, pt/x)*QuenchFF(pt)/x**2
    IDH = 8
	Intf_mesonSS = Intf_mesonSS + Fiqc(IDH, x, centr)*QuenchFiqc(x)*FF(IDH, IDparticle, pt/x)*QuenchFF(pt)/x**2
    !print *, Fiqc(IDH, x, centr),QuenchFiqc(x),FF(IDH, IDparticle, pt/x),QuenchFF(pt)
    else
...
! meson spectrum from SS where SS from 2 jets
	...
	external Intf_pionSS2j, Intf_kaonSS2j, Intf_D0SS2j, Intf_DsSS2j
	real*8 Intf_pionSS2j, Intf_kaonSS2j, Intf_D0SS2j, Intf_DsSS2j
...
!---------------------------------------------	
	...
	call Integral_1d(Intf_D0SS2j, fa, fb, fdel, fp1, fp2, fp3, fpt, fIDP1, fIDP2, fIDP3, fcentr, fsumf)
	!meson_SS2j = 5.0d0*gamma2j/p0/pt**6*fsumf
    meson_SS2j = gbst*12.0d0*gamma2j/p0/pt**5*fsumf
    
    elseif(Isu.eq.10)then
	
	call Integral_1d(Intf_DsSS2j, fa, fb, fdel, fp1, fp2, fp3, fpt, fIDP1, fIDP2, fIDP3, fcentr, fsumf)
	meson_SS2j = gbst*60.0d0*gamma2j/p0/pt**7*fsumf
...
!---------------------------------------------------
! integral function for D0 from SS2j
! inputs, x, pt, IDP1, IDP2, centr
	
	function Intf_D0SS2j(x, p1, p2, p3, pt, IDP1, IDP2, IDP3, centr)
	implicit none
	include 'Recom.h'
	real*8 Intf_D0SS2j
	real*8 x, p1, p2, p3, pt
...
!--------------------------------------------
	...
	!Intf_D0SS2j = (pt-x)**4*HardInt(IDP1, x, centr)*HardInt(IDP2, pt-x, centr)
    Intf_D0SS2j = x*(pt-x)**2*HardInt(IDP1, x, centr)*HardInt(IDP2, pt-x, centr)
...
!---------------------------------------------------
! integral function for Ds from SS2j
! inputs, x, pt, IDP1, IDP2, centr
	
	function Intf_DsSS2j(x, p1, p2, p3, pt, IDP1, IDP2, IDP3, centr)
	implicit none
	include 'Recom.h'
	real*8 Intf_DsSS2j
	real*8 x, p1, p2, p3, pt
...
!--------------------------------------------
	...
	Intf_DsSS2j = x**2*(pt-x)**3*HardInt(IDP1, x, centr)*HardInt(IDP2, pt-x, centr)
...
! Sij from IDH to IDP
	...
	real*8  kns, ll, gg, squ, sqg, kk, A0, a, b, c, d
...
!-----------------------------------------
    ...
    if(IDH.eq.8.or.IDH.eq.9.or.IDP.eq.8.or.IDP.eq.9)then
...
! IDH
! 1 u, 2 d, 3 ubar, 4 dbar, 5 g, 6 s, 7 sbar, 8 c, 9 cbar
! IDparticle
! 1 pi+-, 2 k+-, 3 p+-, 8 Jpsi, 9 D^0, 10 D_s
    ...
    external int_DD0c, int_DD0g, int_DDsc, int_DDsg
    real*8 int_DD0c, int_DD0g, int_DDsc, int_DDsg
...
!--------------------------------------------------------------------------
    ...
    fb=x
    fdel=0.02d0
    call Integral_D(int_DD0g,fa,fb,fdel,x,Dg)
    FF=Dg
    elseif(IDH.eq.8)then
    fa=0.d0
    fb=x
    fdel=0.02d0
    call Integral_D(int_DD0c,fa,fb,fdel,x,Dc)
    FF=Dc
    else
    print *, 'Check the Hard jet ID in FF'
	stop
    ...
    fb=x
    fdel=0.02d0
    call Integral_D(int_DDsg,fa,fb,fdel,x,Dg)
    FF=Dg
    !print *, Dg
    elseif(IDH.eq.8)then
    fa=0.d0
    fb=x
    fdel=0.02d0
    call Integral_D(int_DDsc,fa,fb,fdel,x,Dc)
    FF=Dc
    else
    print *, 'Check the Hard jet ID in FF'
	stop
	...
	print *, 'Check the IDparticle in FF'
	stop
...
!--------------------------------------------------------------------------
	...
	if(IDH.eq.1.or.IDH.eq.2.or.IDH.eq.3)then
    ...
    int_DD0c = ( SPD(8,3,x1)*SPD(8,8,(x-x1)/(1.d0-x1))+       &
    SPD(8,3,x1/(1.d0-x+x1))*SPD(8,8,x-x1) )/2.*5*(x-x1)**4/x**6
    !print *, x,x1,int_DD0c
    return
    end
    ...
    int_DD0g = ( SPD(5,3,x1)*SPD(5,8,(x-x1)/(1.-x1))+       &
    SPD(5,3,x1/(1.-x+x1))*SPD(5,8,x-x1) )/2*5*(x-x1)**4/x**6
    ...
    int_DDsc = ( SPD(8,7,x1)*SPD(8,8,(x-x1)/(1.-x1))+       &
    SPD(8,7,x1/(1.-x+x1))*SPD(8,8,x-x1) )*330*x1**2 *(x-x1)**9/x**13
    ...
    int_DDsg = ( SPD(5,7,x1)*SPD(5,8,(x-x1)/(1.0d0-x1))+       &
    SPD(5,7,x1/(1.0d0-x+x1))*SPD(5,8,x-x1) )*330*x1**2 *(x-x1)**9/x**13
    !write(*,*)(x-x1)/(1-x1),S_ij((x-x1)/(1-x1),5,8)
    return
    end
...
!   consider the gammai depends on q
...
!   For Pb+Pb, 2.76 TeV JPG47, 055102(2020)
    gamai = 2.8d0/(1.0d0+q**2.0/49.0d0) 
        elseif(Isys.eq.3)then
!	For Pb+Pb, 5.02 TeV  Phys. Rev. C 104, 014902 
	gamai = 4.5d0/(1.0d0+q**2.0/49.0d0) 

		else
!	gamai = PAR(1)/(1.0d0+(q/PAR(2))**2) 
        endif
    
    
    elseif(IDH.eq.8 .or. IDH.eq.9)then
	!For charm,	by Gong Huan Jing
    !gamai = PAR(1)/(1.0d0+(q/PAR(2))**2)
		...
		!   For Pb+Pb, 2.76 TeV JPG47, 055102(2020)
		gamai =   2.8d0/(1.0d0+q**2.0/49.0d0)
         !    gamai =   0.891D0/(1.0d0+q**2.0/49.0d0) 
        elseif(Isys.eq.3)then
		!	hypothesize that the parameter at 5.02 for c is similar with 2.76TeV
...
!	gamma_s = gamma_q = gamma_g / 2.0D0
        if(Isys.eq.2)then
            
!   For Pb+Pb, 2.76 TeV JPG47, 055102(2020)
    gamai = 2.8d0/2.0d0/(1.0d0+q**2.0/49.0d0) 
		elseif(Isys.eq.3)then
!	For Pb+Pb, 5.02 TeV  Phys. Rev. C 104, 014902 
	gamai = 4.5d0/2.0d0/(1.0d0+q**2.0/49.0d0)
        else
	!gamai = PAR(1)/2.0d0/(1.0d0+(q/PAR(2))**2) 
		endif
    endif
...
!   ZH2013,Phys. Rev. C 88 044919
	xibari = gamai/gama*0.655d0*(1.0d0-centr-0.32D0*cos(phi)*sin(centr**(0.71d0)*pi)) ! depends on c
    if(IYangZhu.eq.1)then
    xibari = 0.0
    endif
...
!---------------------------------------------------------------------------------------
	...
	Intf_fik = Psi(z)*q**2.0*dexp(2.0d0*z*xibari)*fik(IDH, k, centr)*TabcRatio*2.0d0/pi
...
! the minijet distribution 1/(2piptdptdy)
    ...
    real*8 A, bT, b, c1, d, sigma ,Ncoll!the parameter of fik for charm 
...
! c, cbar quark
	...
	fik =Ncoll* A/sigma * dexp(-bT* DATAN(k/b))/(1+(k/b)**d)**c1
...
!=============================================
		! consider fik dependence on c  
    !     fi(k,c) = TAA(c)/TAA(c = 0.025) * fi(k , c = 0.025)
!==============================================
		if( centr.eq.0.025D0)then
			fik = fik
		elseif (  centr== 0.05D0 )then
			fik = 23.37D0/26.2D0  * fik
        elseif (  centr== 0.075D0 )then
			fik = 20.53D0/26.2D0  * fik
        elseif( centr.eq.0.10D0)then
			fik = 18.95D0/26.2D0  * fik
        elseif( centr.eq.0.15D0)then
			fik = 14.53D0/26.2D0  * fik
        elseif( centr.eq.0.25D0)then
			fik = 8.96D0/26.2D0  * fik
		elseif( centr.eq.0.30D0)then
			fik =  7.109D0/26.2D0  * fik
		elseif( centr.eq.0.35D0)then
			fik = 5.259D0/26.2D0  * fik
		elseif( centr.eq.0.4D0)then 
			fik =  4.058D0/26.2D0 *  fik
        elseif( centr.eq.0.45D0)then 
			fik =  2.857D0/26.2D0 *  fik
        elseif( centr.eq.0.55D0)then 
			fik =  1.427D0/26.2D0 *  fik  

		else
			print *, "TAA(c)/TAA(c = 2.5%) at this centrality of this system have not been calculated"
			stop
        endif
!--------------------------------	
...
!==============================================
        ...
        if( centr.eq.0.025D0) then
			fik = fik
		elseif( centr.eq.0.05D0) then
			fik = 23.26D0/26.08D0 * fik
        elseif( centr.eq.0.15D0) then
			fik = 14.4D0/26.08D0 * fik
        elseif( centr.eq.0.25D0) then
			fik = 8.767D0/26.08D0 * fik
		elseif( centr.eq.0.30D0) then
			fik =  6.927D0/26.08D0 *  fik
        elseif( centr.eq.0.35D0) then
			fik =  5.086D0/26.08D0 *  fik
		elseif( centr.eq.0.4D0) then 
			fik =  3.917D0/26.08D0 *  fik
		elseif( centr.eq.0.45D0) then 
			fik =  2.747D0/26.08D0 *  fik
		else
			print *, "TAA(c)/TAA(c = 2.5%) at this centrality of this system have not been calculated"
			stop
        endif
...
!
	...
	do IK = 1, 3100  ! the maximum of pt 30 GeV
